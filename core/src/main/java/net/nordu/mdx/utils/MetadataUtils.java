...
public class MetadataUtils {
	...
	public static void withAttributes(EntityDescriptorType entity, AttributeCallback cb) {
		...
		if (ea != null && ea.getAssertionArray() != null && ea.getAssertionArray().length > 0) {
			...
			for (AttributeStatementType as : assertion.getAttributeStatementArray()) {
				for (AttributeType attribute : as.getAttributeArray()) {
					for (XmlObject vo: attribute.getAttributeValueArray()) {
						cb.attribute(attribute.getNameFormat(), attribute.getName(), vo.xmlText());
					}
				}
			}
		}
